...
class HTTPTransporterFactory {
	...
	public static function buildClient($client, array $configuration = [], array $constructorArguments = [])
	{
		...
		if (is_string($client))
		{
			...
			if (!class_exists($possibleClass)) throw new Exception("HTTP transporter class \"$possibleClass\" cannot be found!");

			$reflClass = new \ReflectionClass($possibleClass);
			$client = $reflClass->newInstanceArgs($constructorArguments);
		}

		if (!$client instanceof ClientInterface) throw new Exception('Client must be implement \\Buzz\\Client\\ClientInterface interface!');
